...
def create_invoice_xml(data, output_path):
    NSMAP = {
...
# Alternative: If you want to support both scenarios, you can conditionally include line-level TaxTotal
def create_invoice_xml_conditional(data, output_path):
    # ... same code as above until InvoiceLines section ...
    
    # InvoiceLines - Conditionally include TaxTotal based on TaxCurrencyCode
    for line in data.get("InvoiceLines", []):
        il = make_element("cac:InvoiceLine", parent=invoice)
        make_element("cbc:ID", line["ID"], il)
        make_element("cbc:InvoicedQuantity", line["Quantity"], il, unitCode=line["UnitCode"])
        ...
        # Only include line-level TaxTotal if TaxCurrencyCode is NOT provided
        if not data.get("TaxCurrencyCode"):
            line_tax = make_element("cac:TaxTotal", parent=il)
            make_element("cbc:TaxAmount", line["TaxAmount"], line_tax, currencyID=data["DocumentCurrencyCode"])
            make_element("cbc:RoundingAmount", line.get("RoundingAmount", 0.00), line_tax, currencyID=data["DocumentCurrencyCode"])
...
if __name__ == "__main__":
    invoice_data = {
        ...
        "TaxCurrencyCode": "SAR",  # This triggers the simplified tax structure requirement
        "BillingReference": "Invoice Number: 354; Invoice Issue Date: 2021-02-10",
        "AdditionalDocumentReferences": [
            {"ID": "ICV", "UUID": "10"},
            {"ID": "PIH", "Attachment": "NWZlY2ViNjZmZmM4NmYzOGQ5NTI3ODZjNmQ2OTZjNzljMmRiYzIzOWRkNGU5MWI0NjcyOWQ3M2EyN2ZiNTdlOQ==", "FileName": "pih.txt"},
        ],
        ...
        "SupplierVAT": "310724367700003",
        "SupplierName": "SME",
        "SupplierSchemeID": "CRN",
        "SupplierCRN": "1010010000",
    ...
    create_invoice_xml(invoice_data, r"d:/ZatcaPython/templates/invoice.xml")
    print("Invoice XML created successfully!")